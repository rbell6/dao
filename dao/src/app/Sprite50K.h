...
namespace dao {
    ...
    public:
        Sprite50K(): mRootEntity(mShader), Application() {
            mShader->enable();
            mShader->setUniform("projection", glm::ortho(-16.0f, 16.0f, -9.0f, 9.0f, -1.0f, 1.0f));
            for (float y{-9.0f}; y < 9.0f; y += 0.1f) {
                for (float x{-16.0f}; x < 16.0f; x += 0.1f) {
                    mRootEntity.getTileLayer().add(new graphics::Sprite(x, y, 0.09f, 0.09f, glm::vec4(rand() % 1000 / 1000.f, 0, 1, 1)));
                }
            }
        }
